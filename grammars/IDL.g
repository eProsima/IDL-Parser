...
specification [Context context, TemplateManager templatemanager, TemplateGroup maintemplates] returns [boolean returnedValue]
@init{
    ...
    :   import_decl* (definition { dtg=$definition.dtg; if(dtg!=null){maintemplates.setAttribute("definitions", dtg.second()); ctx.addDefinition(dtg.first());} })+
	{
	    $returnedValue = true;
	}
    ;
...
@init {
    ...
    :   AT
        ...
        LEFT_BRACE annotation_member_list[annotation] RIGHT_BRACE
        {
...
@init{
    ...
    :   (
            attr_decl { declvector=$attr_decl.declvector; } SEMICOLON
            {
                for(int count = 0; count < declvector.size(); ++count)
