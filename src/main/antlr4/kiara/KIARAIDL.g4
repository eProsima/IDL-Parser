...
@init {
    ...
    |   type_id_decl SEMICOLON
    |   type_prefix_decl SEMICOLON
	|	annotation_appl aux_export
        {
            $etg=$aux_export.etg;

            if($etg != null && $etg.first() != null)
            {
                if($etg.first() instanceof com.eprosima.idl.parser.tree.TreeNode)
                {
                    ((com.eprosima.idl.parser.tree.TreeNode)$etg.first()).addAnnotation(ctx, $annotation_appl.annotation);
                }
            }
        }
    ;

aux_export returns [Pair<Export, TemplateGroup> etg = null]
